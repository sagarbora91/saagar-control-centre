...
const MUTATION_EVIDENCE_CONTRACTS = Object.freeze({
  auth: Object.freeze({ mutationId: 'auth-one-retry-limit-v1', productionFile: 'www/reauth-policy.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/d1-reauth.test.mjs', expectedAssertion: 'D1 reauthentication limits a single action to one retry' }),
  backupRestore: Object.freeze({ mutationId: 'backup-photo-manifest-count-v1', productionFile: 'www/portable-backup.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/portable-backup.test.mjs', expectedAssertion: 'portable backup round-trips without leaking payload text' }),
  etpPublication: Object.freeze({ mutationId: 'etp-critical-reconciliation-publication-gate-v1', productionFile: 'www/etp-reconciliation-policy.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/etp-reconciliation-policy.test.mjs', expectedAssertion: 'publication refuses missing facts, restored state, incomplete scope and critical failures' }),
  export: Object.freeze({ mutationId: 'export-default-deny-policy-v1', productionFile: 'www/export-control.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/eng04-security.test.mjs', expectedAssertion: 'SEC-08 defaults to disabled and records the denied attempt without prompting' }),
  money: Object.freeze({ mutationId: 'money-reconciliation-delta-direction-v1', productionFile: 'www/etp-reconciliation-policy.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/etp-reconciliation-policy.test.mjs', expectedAssertion: 'INV/SR/BC signs are applied and a mismatch remains visible' }),
  storage: Object.freeze({ mutationId: 'storage-native-batch-bound-v1', productionFile: 'www/storage-core.js', testCommand: 'node --test --test-force-exit --test-reporter=tap tests/native-incremental-storage-runtime.test.mjs', expectedAssertion: 'runtime flush writes only changed records in bounded native batches' })
});
