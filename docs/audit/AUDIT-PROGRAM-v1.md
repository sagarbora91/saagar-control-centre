...
- Product anchor: `9e3d9386a42b794afc4a17b9b647e02f8d22ec3d` (see §1.3 for anchor history).
...
| `9e3d9386a42b794afc4a17b9b647e02f8d22ec3d` | 2026-08-24 | Phase 6J final engineering re-anchor after the owner-approved V6 identity, zero-skip conversion and API-23 Android lint fixes. Earlier anchors remain immutable historical evidence. |
...
Audit output is written outside the audited worktree. A5 mutation capture and the two A9 builds are runner-controlled: the runner creates fresh disposable detached worktrees at the exact target and verifies identity and cleanup. No ambient `node_modules` junction exists and there is no fallback to one: each disposable worktree installs its own dependencies with `npm ci --ignore-scripts --no-audit --no-fund`. A bounded dependency-closure identity (`fileCount`, `totalBytes`, `sha256` — aggregates only, never paths or contents) is measured after install, before Gradle and after the build, and all three must match. Each build runs under its own isolated `GRADLE_USER_HOME`, threaded through the build command so the build actually uses it, with a bounded Gradle distribution identity taken before and after. On Windows, Gradle uses the operating-system trusted-root store unless the caller supplied an explicit Java trust-store setting. The disposable generated wrapper's single `networkTimeout` property is normalized to 120 seconds before its recipe hash is captured, allowing the first distribution fetch without weakening URL or certificate validation. Both builds must agree exactly on the receipt-v2 record; disagreement fails `toolchainMatch`. Each build must start without an `android` directory, bootstrap it with the fixed `npm run add:android` command, and bind post-bootstrap/post-build hashes and configuration into its receipt. The audited tree fingerprint is checked before and after the controlled probes.
...
A7-01 may use the runner-bound `a7-message-contract-census-v2` authority for its narrower inventory claim. Every supported sender/receiver site is included, and dynamic sites are retained as hash-bound contract rows. The census extracts executable `<script>` bodies from HTML, follows direct registered handlers and the named shell-host `message(event)` delegate, and limits lifecycle identity to the `ST_*` shell protocol. Worker request/response types such as the ETP parser protocol remain outside A7 unless they explicitly carry an `ST_*` identity. A resolved `ST_*` identity with an expression-valued payload field is an explicit dynamic shape: the sender contract retains its field name and `expression` kind and a hash-bound site row makes changes comparison-visible. It does not make A7-02 or A7-03 unmeasured under complete syntax-census authority. Unknown message identity, whole-payload, spread or computed-call syntax remains unresolved and keeps the applicable check unmeasured. The runner also supplies `a7-a8-static-syntax-census-v3`, bound to the exact frozen tooling identity, after the scanner's comment/string masking, function and catch boundaries, aliases, computed members, delivery guards, worker/message origins, XHR construction and local-navigation forms have passed their adversarial fixtures. It authorizes a clean result only for that enumerated grammar: an unresolved identity/payload site remains unmeasured (or fail-closed for A8-05), and any definite violation still fails. A7-04 is exempt because it decides over a closed, hash-bound set rather than heuristic discovery. A8-05 is also fail-closed: a discovered remote-capable call whose target cannot be validated is measured policy non-compliance (`fail`), not heuristic absence.
...
For each registered mutation, the frozen helper runs exactly `node --test --test-force-exit --test-reporter=tap <registered-test-file>`. `--test-force-exit` is applied only after the test runner has produced its outcome, preventing a detected mutation from being discarded because its deliberately interrupted runtime left a timer or handle open. Detection requires a non-zero exit and the exact registered TAP subtest line `not ok <number> - <expectedAssertion>`, with `AssertionError` or `ERR_ASSERTION` in that subtest's failure block and no setup/module/syntax failure. A generic command failure, a differently named failing test or external JSON is not mutation detection.
...
```json
{
  ...
  "identity": {
    ...
    "productBaselineSha": "9e3d9386a42b794afc4a17b9b647e02f8d22ec3d",
...
node scripts/audit/run.mjs `
  ...
  --output $auditOutput `
  --product-baseline 9e3d9386a42b794afc4a17b9b647e02f8d22ec3d `
...
```powershell
node scripts/audit/run.mjs --root (Get-Location).Path --output $auditOutput --product-baseline 9e3d9386a42b794afc4a17b9b647e02f8d22ec3d --target-sha $targetSha --audit-tooling-sha $auditToolingSha --mode comparison --baseline-evidence $baselineEvidence --run-tests
```
