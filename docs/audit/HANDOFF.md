...
## Repository knowledge graph — tooling, not an audit gate — 2026-08-23

This section is **navigation tooling only**. It closes no gate, carries no
approval, and changes no product or evidence. It is recorded here because it is
the fastest way for a resuming agent to orient.

A Graphify knowledge graph covers the Retail root at
`V:\Co work\Projects\Retail\graphify-out\` — **7,889 nodes, 14,140 edges,
560 communities**. Query it before grepping:

```
graphify query "<question>"          # scoped subgraph
graphify explain "A10-01"            # one concept and its neighbours
graphify path "<A>" "<B>"            # relationship between two nodes
graphify god-nodes                   # architectural hubs
```

**Every layer was built deterministically. No LLM, no API key, and no part of
this corpus was sent to any external service.** That matters here: the graph
indexes `verification/`, `docs/audit/` and the approval registers, so a
cloud-model pass over it would have exported audit evidence and named approvals.
Repeated local-model attempts (qwen2.5-coder:7b, qwen2.5:14b) produced zero nodes
and were abandoned.

Four layers, each with a re-runnable extractor under `docs/organisation/`:

| Layer | Nodes | Source |
|---|---|---|
| Code + markdown | 6,856 | `graphify update .` (AST) |
| CSS + shell/module HTML | 959 | `css-graph-extractor.py` |
| Shared identifiers | 74 | `entity-graph-extractor.py` |
| Doc citations, test→subject | 618 edges | `xref-graph-extractor.py` |

The identifier layer is the useful one for closure work: gate IDs, ETP report
codes and wave markers are first-class nodes, so `graphify explain "A10-01"`
returns every document *and* every source file that touches it — for A10-01 that
is 22 files spanning `HANDOFF.md`, the 4C.2 closure pack, `a10.mjs` and
`comparison.mjs`.

`.docx/.xlsx/.pdf` reports are indexed via markdown sidecars in `docs/converted/`
(generated by `convert-docs.py` from graphify's own python-docx/openpyxl/pypdf
readers). The 10 ETP synthetic fixtures and the evidence `.json` files are
deliberately excluded as generated test data and records rather than architecture.

**Rebuild order after any code change** — `graphify update .` does not know about
the last three layers and they must be re-applied, then re-clustered:

```
graphify update .
python docs/organisation/css-graph-extractor.py
python docs/organisation/entity-graph-extractor.py
python docs/organisation/xref-graph-extractor.py
graphify cluster-only . --no-label
```

Nothing in `verification/audit/` was read for write, modified, or re-derived.
