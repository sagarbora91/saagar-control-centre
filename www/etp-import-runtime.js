...
(function(root,factory){var api=factory(root);if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.SaagarEtpImportRuntimeFactory=api;})(typeof globalThis!=='undefined'?globalThis:this,function(root){
  ...
  function create(options){
    options=options||{};var profile=options.profile,loader=options.loader,workerParser=options.workerParser,lifecycle=options.lifecyclePolicy,coordinatorApi=options.coordinatorApi,nativeApi=options.nativeApi,reconciliation=options.reconciliationPolicy,core=options.coreContract,registryApi=options.controlRegistryApi,readerApi=options.verifiedReaderApi,cryptoApi=options.crypto||root.crypto;
    if(!profile||!loader||(!workerParser&&options.testOnlySynchronousParser!==true)||!lifecycle||!coordinatorApi||!nativeApi||!reconciliation||!core||!registryApi||!readerApi||profile.VERSION!==core.VERSION)return failure('ETP_RUNTIME_DEPENDENCY_INVALID','CREATE');
    ...
    var preparedFiles=null,parsedReports=null,pending=null;
    var pipeline={
  ...
  function bootstrap(){
    try{
      ...
      return create({profile:root.SaagarEtpRetailProfile,loader:loader,workerParser:worker.client,lifecyclePolicy:root.SaagarEtpStoreLifecyclePolicy,coordinatorApi:root.SaagarEtpImportCoordinator,nativeApi:root.SaagarEtpNativeStore,reconciliationPolicy:root.SaagarEtpReconciliationPolicy,coreContract:root.SaagarEtpCoreContract,controlRegistryApi:root.SaagarEtpControlRegistry,verifiedReaderApi:root.SaagarEtpVerifiedReader,storage:root.localStorage,authorizePublication:function(){return !!(typeof root.SaagarReauth==='function'&&root.SaagarReauth('Publish verified Retail ETP reports'));},plugin:root.Capacitor&&root.Capacitor.Plugins&&root.Capacitor.Plugins.SaagarEtpStore,crypto:root.crypto,datePolicy:{earliestDate:'2024-04-01',asOfDate:root.SaagarEtpCoreContract.indiaDate(Date.now()),maxFutureDays:2}});
