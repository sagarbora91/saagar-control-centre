/* V6 ETP: bounded iframe-side composition for governed E3, E4 and E6 presentations. */
(function(root,factory){'use strict';var api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.SaagarEtpOperationalMount=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function mount(options){
    ...
    var valid=record(roots)&&roots.e3&&roots.e4&&roots.e6&&record(bridge)&&record(bridge.e3)&&record(bridge.e4)&&record(bridge.e6)&&record(presentations)&&record(presentations.e3)&&typeof presentations.e3.create==='function'&&record(presentations.e4)&&typeof presentations.e4.mount==='function'&&record(presentations.e6)&&typeof presentations.e6.mount==='function'&&typeof options.getScope==='function'&&typeof options.getActor==='function';
    if(!valid){if(roots){unavailable(roots.e3,'Governed E3 bridge or presentation dependency is unavailable.');unavailable(roots.e4,'Governed E4 bridge or presentation dependency is unavailable.');unavailable(roots.e6,'Governed E6 bridge or presentation dependency is unavailable.');}return fail('ETP_OPERATIONAL_DEPENDENCY_UNAVAILABLE');}
    var destroyed=false,busy=false,queued=false,unsubscribe=null,e3Result,e4Result,e6Result,activeScope=null,activeActor=null;
    ...
    if(!e3Result||e3Result.ok!==true||!e3Result.controller||typeof e3Result.controller.refresh!=='function'||!e4Result||e4Result.ok!==true||typeof e4Result.refresh!=='function'||!e6Result||e6Result.ok!==true||typeof e6Result.refresh!=='function'){
      if(e3Result&&e3Result.controller&&typeof e3Result.controller.destroy==='function')e3Result.controller.destroy();unavailable(roots.e3,'E3 governed presentation failed closed.');unavailable(roots.e4,'E4 governed presentation failed closed.');unavailable(roots.e6,'E6 governed presentation failed closed.');return fail('ETP_OPERATIONAL_MOUNT_FAILED');
    }
    async function refresh(){
      ...
      return freeze({ok:results[0]&&results[0].ok===true&&results[1]&&results[1].ok===true&&results[2]&&results[2].ok===true,results:freeze(results)});
    }
    function onScopeChange(){if(!destroyed)refresh();}
    if(typeof bridge.subscribeScopeChange==='function'){try{unsubscribe=bridge.subscribeScopeChange(onScopeChange);}catch(_){destroyed=true;e3Result.controller.destroy();clear(roots.e4);clear(roots.e6);unavailable(roots.e3,'Scope coordination is unavailable.');unavailable(roots.e4,'Scope coordination is unavailable.');unavailable(roots.e6,'Scope coordination is unavailable.');return fail('ETP_OPERATIONAL_SCOPE_SUBSCRIPTION_FAILED');}if(typeof unsubscribe!=='function'){destroyed=true;e3Result.controller.destroy();clear(roots.e4);clear(roots.e6);unavailable(roots.e3,'Scope coordination is unavailable.');unavailable(roots.e4,'Scope coordination is unavailable.');unavailable(roots.e6,'Scope coordination is unavailable.');return fail('ETP_OPERATIONAL_SCOPE_SUBSCRIPTION_FAILED');}}
    function destroy(){if(destroyed)return freeze({ok:true,idempotent:true});destroyed=true;queued=false;if(unsubscribe){try{unsubscribe();}catch(_){}}e3Result.controller.destroy();clear(roots.e4);clear(roots.e6);return freeze({ok:true,idempotent:false});}
