...
(function (root, factory) {
  ...
  function seal(payload, passphrase, options) {
    ...
    var iterations = Number(options && options.iterations || DEFAULT_ITERATIONS);
    if (!Number.isInteger(iterations) || iterations < MIN_ITERATIONS || iterations > MAX_ITERATIONS) {
      return Promise.reject(Object.assign(new Error('Backup key-strength setting is invalid.'), { code: 'INVALID_KDF' }));
    }
    var c;
  ...
  function parseContainer(input) {
    ...
    if (value.format !== FORMAT || value.version !== VERSION) fail('UNSUPPORTED_CONTAINER', 'This backup format is not supported by this app version.');
    ...
    if (kdf.name !== 'PBKDF2' || kdf.hash !== 'SHA-256' ||
        ...
        cipher.name !== 'AES-GCM' || cipher.keyBits !== 256 || cipher.tagBits !== 128) {
  ...
  function open(input, passphrase) {
    ...
    try {
      salt = base64ToBytes(container.kdf.salt, 'salt');
      iv = base64ToBytes(container.cipher.iv, 'IV');
      ciphertext = base64ToBytes(container.ciphertext, 'ciphertext');
      if (salt.length !== 16 || iv.length !== 12 || ciphertext.length < 17) fail('INVALID_CONTAINER', 'The backup encryption data is invalid.');
    } catch (err) { return Promise.reject(err); }
    var aad = encoder.encode(canonical(container.manifest));
    return deriveKey(passphrase, salt, container.kdf.iterations, ['decrypt']).then(function (key) {
      ...
      return verifyManifest(payload, bytes, container.manifest).then(function () {
        return { payload: payload, manifest: container.manifest };
      });
    });
  }
