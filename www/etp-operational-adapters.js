...
(function(root,factory){'use strict';var api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.SaagarEtpOperationalAdapters=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function createE4(options){
    ...
    var adapter={
      ...
      getSourceBinding:function(k){ensure(k);var x=get('E4-SOURCE-INTAKE');if(x&&x.kind!=='E4_SOURCE_INTAKE')throw new Error('ETP_E4_ADAPTER_RECORD_INVALID');return x&&rec(x.binding)?clone(x.binding):undefined;},
      saveSourceIntake:function(k,value,operation){ensure(k);if(!rec(value)||!operation||!id(operation.operationId)||!rec(operation.value)||!rec(operation.value.binding))throw new Error('ETP_E4_ADAPTER_RECORD_INVALID');write('E4-SOURCE-INTAKE',{kind:'E4_SOURCE_INTAKE',sources:clone(value),binding:clone(operation.value.binding)});write('E4-OP-'+operation.operationId,{kind:'E4_OPERATION',operation:clone(operation)});},
