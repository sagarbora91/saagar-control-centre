/* V6 ETP Wave 5: production shell composition for governed E3/E4 operations. */
(function(root,factory){'use strict';var api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else if(root)root.SaagarEtpOperationalShellComposer=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function dependencies(r){return rec(r)&&r.SaagarEtpModuleGateway&&r.SaagarEtpModuleGateway.readFacade&&r.SaagarEtpOperationalBootstrap&&r.SaagarEtpOperationalMount&&r.SaagarEtpE3VerifiedJoin&&r.SaagarEtpOperationalFoundation&&r.SaagarEtpOperationalStore&&r.SaagarEtpOperationalRuntime&&r.SaagarEtpOperationalAdapters&&r.SaagarEtpCroReconciliation&&r.SaagarEtpTargetPlanning&&r.SaagarEtpE3Orchestrator&&r.SaagarEtpE4Orchestrator&&r.SaagarEtpOperationalGateway&&r.SaagarEtpE3Presentation&&r.SaagarEtpE4Presentation&&r.SaagarOwnerSession&&typeof r.SaagarOwnerSession.read==='function'&&typeof r.SaagarReauth==='function'&&r.localStorage?r:null;}
  ...
  async function compose(options){
    ...
    if(!scope||!rec(roots)||!roots.e3||!roots.e4)return fail('ETP_SHELL_SCOPE_UNAVAILABLE');
    ...
    var boot;try{boot=r.SaagarEtpOperationalBootstrap.create({foundation:r.SaagarEtpOperationalFoundation,store:r.SaagarEtpOperationalStore,runtime:r.SaagarEtpOperationalRuntime,adapters:r.SaagarEtpOperationalAdapters,e3Engine:r.SaagarEtpCroReconciliation,e4Engine:r.SaagarEtpTargetPlanning,e3Orchestrator:r.SaagarEtpE3Orchestrator,e4Orchestrator:r.SaagarEtpE4Orchestrator,gateway:r.SaagarEtpOperationalGateway,verifiedJoin:verified,storage:r.localStorage,ownerSession:session,reauth:reauth,clock:clock,authorities:{E3:E3_AUTHORITY}});}catch(_){return fail('ETP_SHELL_BOOTSTRAP_FAILED');}
    ...
    var mounted;try{mounted=r.SaagarEtpOperationalMount.mount({roots:roots,bridge:{e3:e3,e4:boot.operational.E4,getVerifiedBinding:function(){return b;}},presentations:{e3:r.SaagarEtpE3Presentation,e4:r.SaagarEtpE4Presentation},getScope:function(){return scope;},getActor:function(){return session();}});}catch(_){return fail('ETP_SHELL_MOUNT_FAILED');}
