...
function prepare(id,row){ return policy.preparePersistableRow(id,row,adapters,policy.detectReport(Object.keys(row),schemas),datePolicy); }
test('reduced synthetic fixtures cover all four policies without claiming source acceptance',()=>{ for(const id of policy.REPORT_IDS) assert.equal(policy.detectReport(schemas[id],schemas).reportId,id); });
test('approved known PII is dropped and its value canary never becomes persistable',()=>{ const out=prepare('R022',{TRANS_TYPE:'INV',STORE_CODE:'WLMHW',INVNUMBER:'0001',INVOICEDATE:'20240916',NETVALUE:'100',ENCIRCLE:'25',CUSTOMERNAME:'PRIVATE-CANARY'}); assert.equal(out.ok,true); assert.equal(out.persistableRow.fields.encircleAmount,'25'); assert.equal('customerName' in out.persistableRow.fields,false); assert.doesNotMatch(JSON.stringify(out.persistableRow),/PRIVATE-CANARY/); });
test('unknown and unapproved known PII still fail closed',()=>{ const extended={...schemas,R022:[...schemas.R022,'SURPRISE']}; const row={TRANS_TYPE:'INV',STORE_CODE:'WLMHW',INVNUMBER:'1',INVOICEDATE:'20240916',NETVALUE:'1',ENCIRCLE:'0',CUSTOMERNAME:'x',SURPRISE:'x'}; const out=policy.preparePersistableRow('R022',row,adapters,policy.detectReport(Object.keys(row),extended),datePolicy); assert.equal(out.ok,false); assert.ok(out.fatalErrors.some(e=>e.code==='FIELD_NOT_WHITELISTED')); });
test('bare ENCIRCLE differs from ENCIRCLE identifier aliases',()=>{ assert.equal(policy.isForbiddenPiiHeader('ENCIRCLE'),false); assert.equal(policy.isForbiddenPiiHeader('ENCIRCLE NO'),true); assert.equal(policy.isForbiddenPiiHeader('ENCIRCLE_NUMBER'),true); });
test('rows bind to the exact detected signature',()=>{ const row={TRANS_TYPE:'INV',STORE_CODE:'WLMHW',INVNUMBER:'1',INVOICEDATE:'20240916',NETVALUE:'1',ENCIRCLE:'0',CUSTOMERNAME:'x'}; const out=policy.preparePersistableRow('R022',row,adapters,policy.detectReport(schemas.R003,schemas),datePolicy); assert.ok(out.fatalErrors.some(e=>e.code==='HEADER_SIGNATURE_MISMATCH')); });
test('report-specific identifiers and measures are mandatory and nonblank',()=>{ const out=prepare('R025',{TRANS_TYPE:'INV',STORE_CODE:'WLMHW',INVNUMBER:'1',ITEMNUMBER:'',INVDATE:'20240916',QTY:'1',NETVALUE:'10'}); assert.ok(out.fatalErrors.some(e=>e.code==='REQUIRED_FIELD_MISSING'&&e.field==='itemNumber')); });
