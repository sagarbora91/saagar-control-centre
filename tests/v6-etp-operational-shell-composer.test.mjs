...
function setup(change={}){let captured={},destroyed=0,reauth=0;const E3={load(){return{ok:true};},lock(){return{ok:typeof captured.bootstrap.reauth('LOCK')==='string'};}};const operational={E3,E4:{load(){}},status:Object.freeze({E3:'READY_WLMHW',E4:'UNAVAILABLE_AUTHORITY_REQUIRED'})};const root={
  ...
  SaagarEtpOperationalFoundation:{},SaagarEtpOperationalStore:{},SaagarEtpOperationalRuntime:{},SaagarEtpOperationalAdapters:{},SaagarEtpCroReconciliation:{},SaagarEtpTargetPlanning:{},SaagarEtpE3Orchestrator:{},SaagarEtpE4Orchestrator:{},SaagarEtpOperationalGateway:{},SaagarEtpE3Presentation:{},SaagarEtpE4Presentation:{},
...
test('composes current verified receipt, Gate-0 E3 authority, durable bootstrap and mount',async()=>{const x=setup(),out=await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>scope});assert.equal(out.ok,true);assert.equal(x.captured.join.binding.receiptId,'receipt-'+'a'.repeat(32));assert.equal(x.captured.bootstrap.authorities.E3.approvalId,'E3-OWNER-2026-08-25-V1');assert.equal(x.captured.bootstrap.authorities.E4,undefined);assert.equal(x.captured.bootstrap.storage,x.root.localStorage);assert.equal(x.captured.mount.bridge.e3,x.captured.bootstrap&&out.ok?x.captured.mount.bridge.e3:null);assert.deepEqual(Object.keys(out).sort(),['controller','ok','status']);assert.equal(out.status.E4,'UNAVAILABLE_AUTHORITY_REQUIRED');});
...
test('fails closed for HEMW, non-owner sessions, unverified receipts and missing dependencies',async()=>{
  ...
  assert.equal((await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>hemw})).code,'ETP_SHELL_SCOPE_UNAVAILABLE');
  x=setup({SaagarOwnerSession:{read:()=>({version:1,isOwner:false,role:'Store Manager'})}});
  assert.equal((await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>scope})).code,'ETP_SHELL_OWNER_SESSION_REQUIRED');
  ...
  assert.equal((await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>scope})).code,'ETP_SHELL_VERIFIED_SCOPE_UNAVAILABLE');
  assert.equal((await composer.compose({root:{},roots:{e3:{},e4:{}},getScope:()=>scope})).code,'ETP_SHELL_DEPENDENCY_UNAVAILABLE');
});

test('trusted owner and one-use reauthentication providers are translated without exposing them',async()=>{const x=setup(),out=await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>scope}),session=x.captured.bootstrap.ownerSession(),locked=await x.captured.mount.bridge.e3.lock({});
...
test('binding adapter rejects drift and initial refresh failure destroys the mount',async()=>{const x=setup(),out=await composer.compose({root:x.root,roots:{e3:{},e4:{}},getScope:()=>scope}),verified=x.captured.bootstrap.verifiedJoin;assert.equal((await verified.readE3({scopeKey:'wrong',generationId:generation,receiptId:'receipt-'+generation.slice(4),businessDate:'2026-08-25'})).code,'E3_VERIFIED_BINDING_INVALID');assert.equal(verified.getNextBinding(),null);const bad=setup({SaagarEtpOperationalMount:{mount:()=>({ok:true,controller:{refresh:async()=>({ok:false}),destroy(){bad.wasDestroyed=true;}}})}});assert.equal((await composer.compose({root:bad.root,roots:{e3:{},e4:{}},getScope:()=>scope})).code,'ETP_SHELL_INITIAL_REFRESH_FAILED');assert.equal(bad.wasDestroyed,true);});
