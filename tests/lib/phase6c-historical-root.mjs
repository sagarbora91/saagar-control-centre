...
`, '')
  ...
  if (commonSource.includes(PHASE6I_MOBILE_MARKER)) {
    ...
    let manifest = fs.readFileSync(manifestPath, 'utf8')
      .replace('input.sharedAssets.length !== 37', 'input.sharedAssets.length !== 38')
      .replace('sharedAssets must contain exactly thirty-seven entries', 'sharedAssets must contain exactly thirty-eight entries')
  ...
  for (const asset of ['etp-verified-analytics.js', 'etp-analytics-consumer.js', 'etp-operational-foundation.js', 'etp-operational-store.js', 'etp-cro-reconciliation.js', 'etp-e3-orchestrator.js', 'etp-target-planning.js', 'etp-e4-orchestrator.js', 'etp-exception-monitor.js', 'etp-incentive-control.js', 'etp-operations-consumer.js']) {
  ...
  let manifestSource = fs.readFileSync(manifestPath, 'utf8')
    .replace("input.sharedAssets.length !== 38", "input.sharedAssets.length !== 27")
    .replace("sharedAssets must contain exactly thirty-eight entries", "sharedAssets must contain exactly twenty-seven entries")
