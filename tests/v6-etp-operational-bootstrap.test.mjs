...
test('E4 becomes callable only with a separately injected approved authority',async()=>{const o=options();o.authorities.E4=authority('E4','e');const api=bootstrap.create(o).operational,loaded=await api.E4.load({scope:{scopeKey}});assert.equal(api.status.E4,'AUTHORITY_INJECTED');assert.equal(loaded.status,'READY');assert.equal(loaded.authorities.length,4);assert.equal((await api.E4.publish({scope:{scopeKey},input:{operationId:'p1'}})).ok,true);});
...
test('public surface exposes only frozen E3, E4 and status without repository or raw/native capabilities',()=>{const api=bootstrap.create(options()).operational;assert.deepEqual(Object.keys(api).sort(),['E3','E4','status']);const serialized=JSON.stringify(api);assert.doesNotMatch(serialized,/repository|storage|native|raw|verifiedJoin/);const source=fs.readFileSync(new URL('../www/etp-operational-bootstrap.js',import.meta.url),'utf8');assert.doesNotMatch(source,/localStorage|sessionStorage|indexedDB|Capacitor|SaagarEtpNativeStore|window\.|document\.|readFacts|rawRows|sourceFacts|fetch\s*\(|XMLHttpRequest|\?\.|\?\?/);});
