...
test('public surface exposes only frozen E3, E4, E6 and status without repository or raw/native capabilities',()=>{const api=bootstrap.create(options()).operational;assert.deepEqual(Object.keys(api).sort(),['E3','E4','E6','status']);assert.equal(api.status.E6,'UNAVAILABLE_AUTHORITY_REQUIRED');const serialized=JSON.stringify(api);assert.doesNotMatch(serialized,/repository|storage|native|raw|verifiedJoin/);const source=fs.readFileSync(new URL('../www/etp-operational-bootstrap.js',import.meta.url),'utf8');assert.doesNotMatch(source,/localStorage|sessionStorage|indexedDB|Capacitor|SaagarEtpNativeStore|window\.|document\.|readFacts|rawRows|sourceFacts|fetch\s*\(|XMLHttpRequest|\?\.|\?\?/);});
